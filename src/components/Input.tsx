...
const Input: React.FC<InputProps> = ({
     ...
     return (
          <div className={`relative ${className} `}>
               ...
               />
               <label
                    className={`absolute top-2 bg-zinc-800 px-1 left-4 z-[1] duration-100 -translate-y-5 transform origin-[0px] peer-placeholder-shown:scale-100
                    peer-placeholder-shown:translate-y-0
                    peer-focus:-translate-y-5
                    peer-focus:scale-75 ${error[id] ? 'text-red-500 ' : 'text-zinc-500'} `}
                    htmlFor={id}
               >
                    {label}
               </label>
